...
data class LanguageLayout(
    ...
    companion object {
        fun getEnglishLayout(): LanguageLayout {
            val row1 = KeyRow(listOf("q", "w", "e", "r", "t", "y", "u", "i", "o", "p").map { KeyData(it, it) })
            val row2 = KeyRow(listOf("a", "s", "d", "f", "g", "h", "j", "k", "l").map { KeyData(it, it) })
            val row3 = KeyRow(listOf("z", "x", "c", "v", "b", "n", "m").map { KeyData(it, it) })
